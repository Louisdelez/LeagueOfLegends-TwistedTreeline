...
use crate::ability_plugin::{AbilityCooldowns, ChampionKit};
...
fn spawn_champion_on_enter(
    ...
    let model_path = match class {
        ChampionClass::Mage => Some("models/champions/annie.glb#Scene0"),
        ChampionClass::Fighter => Some("models/champions/garen.glb#Scene0"),
        ChampionClass::Tank => None,
    };

    let champion_components = (
        Champion {
        ...
        CombatStats {
            attack_damage: def.ad,
            ability_power: def.ap,
            armor: def.armor,
            magic_resist: def.mr,
        ...
        BaseStats {
            ...
            attack_speed: def.attack_speed,
            move_speed: def.move_speed,
            ad_per_level: def.ad_per_level,
            armor_per_level: def.armor_per_level,
            mr_per_level: def.mr_per_level,
            hp_per_level: def.hp_per_level,
            mana_per_level: def.mana_per_level,
            base_hp: def.hp,
            base_mana: def.mana,
        },
        KillStreak::default(),
    ...
    // Spawn 2 allied bots + 3 enemy bots
    let bot_configs: [(Team, ChampionClass, Lane, Vec3, [f32; 3]); 5] = [
        // 2 Blue allies
        (Team::Blue, ChampionClass::Fighter, Lane::Top, Vec3::new(1059.0, 55.0, 7200.0), [0.2, 0.5, 0.9]),
        (Team::Blue, ChampionClass::Tank, Lane::Bottom, Vec3::new(1059.0, 55.0, 7400.0), [0.1, 0.4, 0.7]),
        // 3 Red enemies
        (Team::Red, ChampionClass::Mage, Lane::Top, Vec3::new(14321.0, 55.0, 7135.0), [0.9, 0.2, 0.2]),
        (Team::Red, ChampionClass::Fighter, Lane::Bottom, Vec3::new(14321.0, 55.0, 7335.0), [0.8, 0.3, 0.1]),
        (Team::Red, ChampionClass::Tank, Lane::Top, Vec3::new(14321.0, 55.0, 7435.0), [0.7, 0.1, 0.1]),
    ];

    for (bot_team, bot_class, bot_lane, bot_pos, bot_color) in bot_configs {
        let bot_def = get_champion(bot_class);
        let bot_mesh = meshes.add(Capsule3d::new(25.0, 60.0));
        let bot_mat = materials.add(StandardMaterial {
            base_color: Color::srgb(bot_color[0], bot_color[1], bot_color[2]),
            emissive: bevy::color::LinearRgba::rgb(bot_color[0] * 0.3, bot_color[1] * 0.3, bot_color[2] * 0.3),
            ..default()
        });
        ...
        commands.spawn((
            ...
            CombatStats {
                attack_damage: bot_def.ad,
                ability_power: bot_def.ap,
                armor: bot_def.armor,
                magic_resist: bot_def.mr,
                attack_speed: bot_def.attack_speed,
                move_speed: bot_def.move_speed,
                crit_chance: 0.0, cdr: 0.0,
                armor_pen_flat: 0.0, armor_pen_pct: 0.0,
                magic_pen_flat: 0.0, magic_pen_pct: 0.0,
                life_steal: 0.0, spell_vamp: 0.0,
            ...
            BaseStats {
                ...
                attack_speed: bot_def.attack_speed,
                move_speed: bot_def.move_speed,
                ad_per_level: bot_def.ad_per_level,
                armor_per_level: bot_def.armor_per_level,
                mr_per_level: bot_def.mr_per_level,
                hp_per_level: bot_def.hp_per_level,
                mana_per_level: bot_def.mana_per_level,
                base_hp: bot_def.hp,
                base_mana: bot_def.mana,
            },
            KillStreak::default(),
            GameStats::default(),
        ));
